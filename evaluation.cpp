...
/*else if (desc_type == DESC_SPIN_IMAGE)
{
  ...
  Feature<PointXYZRGB, Histogram<153> >::Ptr feature_extractor(new SpinImageEstimation<PointXYZRGB, PointNormal, Histogram<153> >);

  // Scene
  feature_extractor->setInputNormals(scene_normals);
  feature_extractor->setSearchSurface(scene_cloud);
  feature_extractor->setInputCloud(scene_keypoints);
  search::KdTree<PointRGB>::Ptr kdtree(new search::KdTree<PointRGB>);
  feature_extractor->setSearchMethod(kdtree);
  feature_extractor->setRadiusSearch(v_desc_radius_search[j]);
  feature_extractor->compute(*scene_features);

  // Object
  feature_extractor->setInputNormals(object_normals);
  feature_extractor->setSearchSurface(object_cloud);
  feature_extractor->setInputCloud(object_keypoints);
  feature_extractor->setSearchMethod(kdtree);
  feature_extractor->setRadiusSearch(v_desc_radius_search[j]);
  feature_extractor->compute(*object_features);
  ...
  cout << "Keypoints Radius: " << fixed << v_kpts_radius_search[i] << endl;
  cout << "Descriptor Radius: " << v_desc_radius_search[j] << endl;
  cout << "Inlier Threshold: " << v_inliers_threshold[h] << endl;
